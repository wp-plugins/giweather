...
function GIWeatherWidget(giweather_element,options)
{
  ...
  this.setCurrentThumb=function(thumb){
		...
		el=jQuery('.giw-fc-thumb-pressure-val',thumb);
		if (el.length==1){
			jQuery('.giw-fc-cur-pressure > span',giweather_element).text(el.text());
			jQuery('.giw-fc-cur-pressure',giweather_element).show();
		}else{
			jQuery('.giw-fc-cur-pressure',giweather_element).hide();
		}
		
		el=jQuery('.giw-fc-thumb-wind-val',thumb);
		if (el.length==1){
			jQuery('.giw-fc-cur-wind > span',giweather_element).text(el.text());
